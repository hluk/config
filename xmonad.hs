...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm              , xK_Tab     ), windowPromptGoto defaultXPConfig)
...
myManageHook = composeAll
    ...
    , className =? "Gimp"           --> (doFloat <+> doF (W.shift "3"))
    , className =? "Chrome"         --> doF (W.shift "  2  ")
...
main = xmonad =<< statusBar myBar myPP toggleStrutsKey (withUrgencyHook NoUrgencyHook (ewmh myConfig))

-- Command to launch the bar.
myBar = "xmobar"

-- Custom PP, configure it as you like. It determines what's being written to the bar.
myPP = xmobarPP { ppCurrent = xmobarColor "#ffffff" "#0066ee" }
